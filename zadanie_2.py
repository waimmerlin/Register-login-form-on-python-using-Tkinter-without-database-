...
def registration():
    ...
    password1.place(relx=0.4, rely=0.4,
                    relheight=0.1, relwidth=0.55)
    # Confirm password
    label_password2 = Label(frame, text='Confirm pw: ')
        ...
        if len(login_register.get()) == 0:
            ...
            try:
                with open('login.txt', 'rb') as f:
                    data = pickle.load(f)
                if login_register.get() in data:
                    error = '*login already exists. Create new or Sign In'
                else:
                ...
                # if login.txt file doesn't exist, create it and save the new login
                data = {login_register.get(): password1.get()}
                with open('login.txt', 'wb') as f:
                    pickle.dump(data, f)
                login_form()
